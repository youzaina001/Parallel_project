...
contains
  ...
  subroutine mat_coeff(dx,dy,alpha,beta,gamma)
    ...
    if (alpha_robin == 0) then
      ...
      if(me /= 0) then
        ...
        do i = iBeg, iBeg+Nx-1
          ...
          res(i) = (1._PR + D*dt*(2._PR/(dx**2)+2._PR/(dy**2)+2._PR*beta_robin/(alpha_robin*dy)))*x(i)
    ...
    if(alpha_robin == 0) then
      ...
      if (Me /= 0) then
        ...
        do i = iBeg, iBeg+Nx-1
          ...
          b(i) = b(i) + D*dt*(2._PR*beta_robin*Up(i-iBeg+1+Nx)/(alpha_robin*dy)+(Up(i-iBeg+1)-Up(i-iBeg+1+2*Nx))/(dy)**2)
      ...
      if (me /= (Np-1)) then
        ...
        do i = iEnd-Nx+1, iEnd
          ...
          b(i) = b(i) + D*dt*(2._PR*beta_robin*Down(i-iEnd+2*Nx)/(alpha_robin*dy)+(Down(i-iEnd+3*Nx)-Down(i-iEnd+Nx))/(dy)**2)
